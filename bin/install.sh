...
install_lazyvim() {
    ...
    case "$PACKAGE_MANAGER" in
        brew)
            ...
            ido brew update
            ido brew install nvim node@24 || die "failed to install lazyvim and deps"
        ...
        apt)
            ...
            ido "curl -fsSL https://deb.nodesource.com/setup_lts.x | sudo -E bash -" || die "failed to setup nodejs repository"
